...
browser.runtime.onMessage.addListener(request => {
  ...
  //setTimeout(calc_strength, 1000);
  //console.log("Send response to sidebar script");
  //console.log(window.calc_response);
  //return Promise.resolve([1,2]);//window.calc_response);
});
  ...
  function script() {
    ...
    window.addEventListener("message", function(event) {
      ...
      if (event.data.type && (event.data.type == "FROM_SCRIPT")) {
        console.log("Web page script received: ");
